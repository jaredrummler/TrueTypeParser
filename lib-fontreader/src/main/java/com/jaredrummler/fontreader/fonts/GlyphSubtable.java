...
public abstract class GlyphSubtable implements Comparable {
   ...
   *
   * @param lookupId
   *     lookup identifier, having form of "lu%d" where %d is index of lookup in lookup list
   * @param sequence
   *     subtable sequence (within lookup), starting with zero
   * @param flags
   *     subtable flags
   * @param format
   *     subtable format
   ...
   *     if table is already set to non-null
   */
